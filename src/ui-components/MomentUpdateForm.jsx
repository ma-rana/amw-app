...
export default function MomentUpdateForm(props) {
  const {
    id: idProp,
    moment: momentModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = momentRecord
      ? { ...initialValues, ...momentRecord }
      : initialValues;
    ...
    setTaggedUserIds(cleanValues.taggedUserIds ?? []);
    setCurrentTaggedUserIdsValue("");
    setTimestamp(cleanValues.timestamp);
    setStoryId(cleanValues.storyId);
    setReportedCount(cleanValues.reportedCount);
    setErrors({});
  };
  const [momentRecord, setMomentRecord] = React.useState(momentModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Moment, idProp)
        : momentModelProp;
      setMomentRecord(record);
    };
    queryData();
  }, [idProp, momentModelProp]);
  React.useEffect(resetStateValues, [momentRecord]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Moment.copyOf(momentRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "MomentUpdateForm")}
      {...rest}
    >
      <TextField
        label="Title"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || momentModelProp)}
