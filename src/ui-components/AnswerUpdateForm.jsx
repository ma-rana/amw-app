...
export default function AnswerUpdateForm(props) {
  ...
  const resetStateValues = () => {
    ...
    setAttachmentIds(cleanValues.attachmentIds ?? []);
    setCurrentAttachmentIdsValue("");
    setErrors({});
  };
  ...
  const [currentAttachmentIdsValue, setCurrentAttachmentIdsValue] =
    React.useState("");
  const attachmentIdsRef = React.createRef();
  const validations = {
    userId: [],
    questionId: [],
    text: [],
      ...
      <ArrayField
        onChange={async (items) => {
          ...
          if (onChange) {
            ...
            const result = onChange(modelFields);
            values = result?.attachmentIds ?? values;
          }
          setAttachmentIds(values);
          setCurrentAttachmentIdsValue("");
        }}
        currentFieldValue={currentAttachmentIdsValue}
        label={"Attachment ids"}
        items={attachmentIds}
        hasError={errors?.attachmentIds?.hasError}
        runValidationTasks={async () =>
          await runValidationTasks("attachmentIds", currentAttachmentIdsValue)
        }
        errorMessage={errors?.attachmentIds?.errorMessage}
        setFieldValue={setCurrentAttachmentIdsValue}
        inputFieldRef={attachmentIdsRef}
        defaultFieldValue={""}
      >
        <TextField
          label="Attachment ids"
          isRequired={false}
          isReadOnly={false}
          value={currentAttachmentIdsValue}
          onChange={(e) => {
            let { value } = e.target;
            if (errors.attachmentIds?.hasError) {
              runValidationTasks("attachmentIds", value);
            }
            setCurrentAttachmentIdsValue(value);
          }}
          onBlur={() =>
            runValidationTasks("attachmentIds", currentAttachmentIdsValue)
          }
          errorMessage={errors.attachmentIds?.errorMessage}
          hasError={errors.attachmentIds?.hasError}
          ref={attachmentIdsRef}
          labelHidden={true}
          {...getOverrideProps(overrides, "attachmentIds")}
        ></TextField>
      </ArrayField>
      <Flex
